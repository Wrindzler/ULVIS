...
function shouldReleaseHardwareAllocation(previousStatus, nextStatus) {
  return previousStatus === 'Kullanımda' && ['Müsait', 'Arızalı'].includes(nextStatus);
}

function releaseActiveHardwareAllocations(database, hardwareId) {
  const info = database.prepare(`
    UPDATE zimmetler
    SET onay_durumu = 'İade Edildi', iade_tarihi = date('now')
    WHERE varlik_tipi = 'Donanım'
      AND varlik_id = ?
...
router.put('/:id', authenticateToken, authorizeRoles('IT Destek', 'IT Müdürü'), (req, res) => {
  try {
    ...
    const nextDurum = durum || item.durum;
    const updateHardware = db.transaction(() => {
      db.prepare(`
      ...
      if (shouldReleaseHardwareAllocation(item.durum, nextDurum)) {
        releaseActiveHardwareAllocations(db, req.params.id);
      }
    });
...
router.shouldReleaseHardwareAllocation = shouldReleaseHardwareAllocation;
router.releaseActiveHardwareAllocations = releaseActiveHardwareAllocations;
