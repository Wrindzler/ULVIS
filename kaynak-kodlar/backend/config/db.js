...
function initializeDatabase() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS yazilim_lisanslari (
      ...
      koltuk_sayisi INTEGER DEFAULT 1 CHECK(koltuk_sayisi >= 1),
  ...
  const licenseCols = db.prepare(`PRAGMA table_info(yazilim_lisanslari)`).all();
  const hasSeatCount = licenseCols.some((c) => c.name === 'koltuk_sayisi');
  if (!hasSeatCount) {
    db.exec(`ALTER TABLE yazilim_lisanslari ADD COLUMN koltuk_sayisi INTEGER DEFAULT 1 CHECK(koltuk_sayisi >= 1)`);
  }
}
