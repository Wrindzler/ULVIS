...
export default function Licenses() {
  ...
  const [form, setForm] = useState({ yazilim_adi: '', lisans_anahtari: '', baslangic_tarihi: '', bitis_tarihi: '', maliyet: '', koltuk_sayisi: 1, durum: 'Aktif' });
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ yazilim_adi: '', lisans_anahtari: '', baslangic_tarihi: '', bitis_tarihi: '', maliyet: '', koltuk_sayisi: 1, durum: 'Aktif' });
    setModalOpen(true);
  };
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      maliyet: item.maliyet || '', koltuk_sayisi: item.koltuk_sayisi || 1, durum: item.durum
    });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = {
        ...
        koltuk_sayisi: Math.max(1, parseInt(form.koltuk_sayisi, 10) || 1),
      };
      if (editing) {
    ...
    <div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="table-header">Kullanım</th>
            ...
            <tbody className="divide-y divide-gray-100">
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">
                      <span className="badge bg-blue-100 text-blue-700">
                        {item.zimmet_sayisi || 0}/{item.koltuk_sayisi || 1}
                      </span>
                    </td>
                    <td className="table-cell">
              ...
              {filteredItems.length === 0 && (
                <tr><td colSpan={10} className="table-cell text-center text-gray-400 py-8">Kayıt bulunamadı</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Lisans Düzenle' : 'Yeni Lisans Ekle'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Kişi Limiti</label>
            <input
              type="number"
              min="1"
              className="input-field"
              value={form.koltuk_sayisi}
              onChange={e => setForm({...form, koltuk_sayisi: e.target.value})}
              required
            />
          </div>
